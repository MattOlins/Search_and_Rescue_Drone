...
const float ALPHA = 0.3f; //30% filter
...
void test_imu()
{
    ...
    //calibrate_sensors(); 
    uint64_t last = get_absolute_time();
    while (1)
    {
        gyro_signals();
        accel_signals();
        ...
        float dt = (float)absolute_time_diff_us(last, now) / 1e6f;
        last = now;
        
        comp_filter_angle(dt);

        printf("Gyro: Roll %.2f  Pitch %.2f  Yaw %.2f\n",
               LPF_RateRoll, LPF_RatePitch, LPF_RateYaw);
...
void test_pid()
{
    ...
    */
    ...
    while (1){
        ...
        // ===== DEBUG PRINT 1: Show raw sensor values =====
        printf("\n--- SENSOR VALUES ---\n");
        printf("Gyro Rates (raw): roll=%.4f pitch=%.4f yaw=%.4f\n",
               LPF_RateRoll, LPF_RatePitch, LPF_RateYaw);
        printf("Accel Angles: roll=%.2f pitch=%.2f\n", MeasRoll, MeasPitch);
        printf("dt = %.4f seconds\n", dt);

       

        // Call controller
        controller_update(dt, altitude, motors, 0);

       

        // ===== DEBUG PRINT 2: Show motor outputs =====
        printf("--- MOTOR OUTPUTS ---\n");
        printf("Motors: FR=%.2f FL=%.2f BR=%.2f BL=%.2f \n", motors[1], motors[3], motors[0], motors[2]);


       

        sleep_ms(100);  // Faster updates for debugging
...
void test_motors()
{
    ...
    while (1)
    {
        ...
        uint64_t now = get_absolute_time();
        float dt = (float)absolute_time_diff_us(last, now) / 1e6f;
        last = now;  // Update for next iteration
        comp_filter_angle(dt);
