...
authorized_names = ["Matt", "Melissa", "Katie"]  #HAS TO BE UPDATED
...
def send_email(name):
	...
	recipient_email = "creanf@lafayette.edu" #CHANGES EMAIL WHERE PHOTOS SENT TO
	app_password = "dfrl mmta uvma gzcz" 
	msg["From"] = sender_email
	msg["To"] = recipient_email
	msg["Subject"] = f"Drone has found {name}!"
	html_body = f"""
<html>
  <body style="font-family: Arial, sans-serif;">
    <p>Hello <strong>{name}</strong>,</p>
    <p>You have been found by the drone. Please review the image below.</p>
    <img src="cid:drone_image" alt="Drone Photo" style="max-width: 100%; border-radius: 6px;">
  </body>
</html>
"""
...
def process_frame(frame): 
    global face_locations, face_encodings, face_names, email_sent, frame_count
    skip_frames = 3
    
    #attempting to add a feature that only runs facial detection every X frames
    
    resized_frame = cv2.resize(frame, (0, 0), fx=(1/cv_scaler), fy=(1/cv_scaler)) #resized frame with cv scaler. higher cv faster fps
    rgb_resized_frame = cv2.cvtColor(resized_frame, cv2.COLOR_BGR2RGB) #opencv uses rgb not bgr
    
    # Find all the faces and face encodings in the current frame of video
    face_locations = face_recognition.face_locations(rgb_resized_frame)
    face_encodings = face_recognition.face_encodings(rgb_resized_frame, face_locations, model='large')
    
    run_detection = (frame_count%skip_frames == 0) #change to increase off frames!
    frame_count+=1
    if run_detection:
	    face_names = []
